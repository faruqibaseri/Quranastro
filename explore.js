...
if (burger && mobile) {
  ...
  // --- Optimization: Disable performance-heavy animations on mobile (max-width: 902px) ---
  const isDesktop = window.matchMedia('(min-width: 903px)').matches;

  /* Scroll reveal */
  if (isDesktop) { // Only enable on desktop
...
// -----------------------------------------------------------------------------------
  ...
  // Gallery configuration
    ...
    function initGallery() {
      ...
      // NOTE: galleryAssembly element is not present in explore.html, 
      // but keeping this function for completeness.
      if (!gallery) return; 
      ...
      // Optimization: Only start rotation on desktop (Already present in original)
      const isDesktopForGallery = window.matchMedia('(min-width: 903px)').matches;
      if (isDesktopForGallery) {
        startRotation();
      }
    }
...
document.querySelectorAll(".mobile__toggle").forEach(btn => {
  ...
  function playAndRedirect(url) {
    ...
    if(sound) { // Added check for sound element
      sound.currentTime = 0;
      sound.play();
    }
